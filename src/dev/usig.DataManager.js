...
usig.DataManager = new function() {
		...
		if (typeof(registeredClasses[className]) != "undefined") {
			if (this.isCached(className, id)) {
				...
				if (typeof(registeredClasses[className].getter) == "function") {
					if (opts.debug) {
						usig.debug('Calling getter with params: '+className+'/'+id);
						usig.debug(params);
					}
					if (typeof(params.success) == "function")
					...
					if (opts.debug) {
						usig.debug('Trying to make Ajax request with params: '+className+'/'+id);
						usig.debug(params);
					}
			...
			if (opts.debug)
				usig.debug('La clase '+className+' no está registrada.');
		}
	};
	...
	function onAjaxSuccess(data, className, id, receiver) {
		...
		if (opts.debug) {
			usig.debug('Received data for: '+className+'/'+id);
			usig.debug(data);
		}
	...
	function onAjaxTimeout(requester, className, id, params) {
		if (requester != null && requester.readyState != 0 && requester.readyState != 4) {
			if (opts.debug)
				usig.debug('Aborting request...');
			requester.abort();
	       	var ajaxReq = $.ajax(params);
			if (opts.debug)
				usig.debug('Retrying request...');
	...
	function mkAjaxRequest(className, id, params) {
		if (typeof(params.success) != "function") {
			if (opts.debug)
				usig.debug('params.success tiene que ser una función que acepte como parámetro el dato solicitado porque puede no estar inmediatamente disponible.');
			return;
		}
		if (typeof(params.error) != "undefined" && typeof(params.error) != "function") {
			if (opts.debug)
				usig.debug('params.error tiene que ser una función.');
			return;
		}
       	...
       	if (typeof(ajaxParams.url) == "undefined") {
       		if (opts.debug) 
       			usig.debug('Falta definir ajaxParams.url para la clase: '+className);
       		return;
       	}
